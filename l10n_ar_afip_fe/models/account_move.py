...
class AccountMove(models.Model):
    ...
    l10n_ar_afip_fe = fields.Selection(related="journal_id.l10n_ar_afip_fe")
    ...
    def _post(self, soft=True):
        ...
        for inv in sale_ar_edi_invoices:
            ...
            client, auth, transport = inv.company_id._l10n_ar_get_connection(inv.journal_id.l10n_ar_afip_fe)._get_client(return_transport=True)
            validated += super(AccountMove, inv)._post()
            return_info = inv._l10n_ar_do_afip_fe_request_cae(client, auth, transport)
            if return_info:
                error_invoice = inv
                validated -= inv
                break
...
""",
    ...
    def _l10n_ar_do_afip_fe_request_cae(self, client, auth, transport):
        ...
        * Result (Approved, Aproved with Observations)
        ...
        """
        for inv in self.filtered(lambda x: x.journal_id.l10n_ar_afip_fe and not x.l10n_ar_afip_auth_code):
            afip_fe = inv.journal_id.l10n_ar_afip_fe
            errors = obs = events = ''
            request_data = False
            return_codes = []
            values = {}
            ...
            if afip_fe == 'wsfe':
                ...
                if result.Resultado != 'A':
                    ...
                    values = {'l10n_ar_afip_auth_code': result.Cae,
                              ...
                              'l10n_ar_afip_auth_code_due': datetime.strptime(result.Fch_venc_Cae, '%Y%m%d').date()}
            return_info = inv._prepare_return_msg(afip_fe, errors, obs, events, return_codes)
    ...
    def _get_related_invoice_data(self):
        ...
        if not related_inv or self.journal_id.l10n_ar_afip_fe == 'wsbfe':
            return res
        ...
        res.update({wskey[afip_fe]['type']: related_inv.l10n_latam_document_type_id.code,
                    wskey[afip_fe]['pos_number']: related_inv.journal_id.l10n_ar_afip_pos_number,
                    wskey[afip_fe]['number']: self._l10n_ar_get_document_number_parts(
        ...
        if self._is_mipyme_fce_refund() or afip_fe == 'wsfex':
            res.update({wskey[afip_fe]['cuit']: int(related_inv.company_id.vat)})

        # WSFE_10158 send orignal invoice date on an mipyme document
        if afip_fe == 'wsfe' and (self._is_mipyme_fce() or self._is_mipyme_fce_refund()):
            res.update({wskey[afip_fe]['date']: related_inv.invoice_date.strftime(WS_DATE_FORMAT[afip_fe])})
    ...
    def _prepare_return_msg(self, afip_fe, errors, obs, events, return_codes):
        self.ensure_one()
        msg = ''
        if any([errors, obs, events]):
            ...
            for code in return_codes:
                fix = afip_errors._hint_msg(code, afip_fe)
                if fix:
    ...
    def wsfex_get_cae_request(self, last_id, client):
        if not self.commercial_partner_id.country_id:
            raise UserError(_('For WS "%s" country is required on partner', self.journal_id.l10n_ar_afip_fe))
        elif not self.commercial_partner_id.country_id.code:
            raise UserError(_('For WS "%s" country code is mandatory country: %s', self.journal_id.l10n_ar_afip_fe, self.commercial_partner_id.country_id.name))
        elif not self.commercial_partner_id.country_id.l10n_ar_afip_code:
            hint_msg = afip_errors._hint_msg('country_afip_code', self.journal_id.l10n_ar_afip_fe)
